...
# 基于tood_coordattention_20_head.py修改,20版本的空间注意力的计算保持由纵向和横向的最大池化+平均池化作为输入，但池化的对象是inter_feat的输入+inter_feat输出，相当于residual
# 本版本通过1x1卷积层负责proposal feat部分,与class分支和regression分支初步解耦的特征进行注意力计算,两任务共用一个1x1卷积层,达到交互作用
...
class InterConvsModule(nn.Module):
    def __init__(self, inter_convs):
        super().__init__()
...
def print_inter_convs_flops(inter_convs, in_channels, h, w):
    model = InterConvsModule(inter_convs)
    input_res = (in_channels, h, w)
    flops, params = get_model_complexity_info(model, input_res, as_strings=True)
    print(f'Inter_convs FLOPs: {flops}, Params: {params}')


def print_task_decomposition_flops(feat_channels, stacked_convs, la_down_rate, conv_cfg, norm_cfg, h, w):
    ...
    input_res = (feat_channels * stacked_convs, h, w)  # 输入通道与forward一致
    flops, params = get_model_complexity_info(model, input_res, as_strings=True)
    print(f'TOODIB_TaskDecomposition FLOPs: {flops}, Params: {params}')
# ...existing code...


@MODELS.register_module()
class TOOD_COORDATT_20_TI_10_Head_test(ATSSHead):
    ...
    def _init_layers(self) -> None:
        ...
        self.need_visualize = False
        
        print_inter_convs_flops(self.inter_convs, self.in_channels, 100, 136)
        #原版ICL为stacked_convs=6, num_dcn=0, 输出为Inter_convs FLOPs: 48.21 GMac, Params: 3.54 M
        #本版本IICL为stacked_convs=4, num_dcn=4, 输出为Inter_convs FLOPs: 13.6 GMac, Params: 3.36 M
        print_task_decomposition_flops(
            feat_channels=self.feat_channels,
            stacked_convs=self.stacked_convs,
            la_down_rate=self.stacked_convs * 8,  # 与你的构造参数一致
            conv_cfg=self.conv_cfg,
            norm_cfg=self.norm_cfg,
            h=100, w=136  # 输入尺寸可根据实际情况调整
                                        )
        # ...existing code...
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[List[Tensor]]:
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            if self.need_visualize:
                #  # ====== 热力图输出部分 ======
                # # 取第一个batch，第一个通道
                heatmap = feat.mean(dim=1)[0]
                heatmap = heatmap.detach().cpu().numpy()
                plt.imshow(heatmap, cmap='jet')
                plt.colorbar()
                plt.title(f'TOODIB_Inter_feats Heatmap mean (level {idx})')
                plt.savefig(f'TOODIB_inter_feats_heatmap_mean_level{idx}.png')
                plt.close()
                # # ===========================
                # # ...existing code...
            ...
            if self.need_visualize:
                # # ====== 热力图输出部分 ======
                # # 取第一个batch，第一个通道
                heatmap = cls_feat.mean(dim=1)[0]
                heatmap = heatmap.detach().cpu().numpy()
                plt.imshow(heatmap, cmap='jet')
                plt.colorbar()
                plt.title(f'TOODIB_TRSD_cls_feats Heatmap mean (level {idx})')
                plt.savefig(f'TOODIB_TRSD_cls_feats_heatmap_mean_level{idx}.png')
                plt.close()

                heatmap = reg_feat.mean(dim=1)[0]
                heatmap = heatmap.detach().cpu().numpy()
                plt.imshow(heatmap, cmap='jet')
                plt.colorbar()
                plt.title(f'TOODIB_TRSD_reg_feats Heatmap mean (level {idx})')
                plt.savefig(f'TOODIB_TRSD_reg_feats_heatmap_mean_level{idx}.png')
                plt.close()
                # # ===========================
                # # ...existing code...
            ...
            if self.need_visualize:
                ...
                plt.imshow(heatmap, cmap='jet')
                plt.colorbar()
                plt.title(f'TOODIB_IBB_cls_feats Heatmap mean (level {idx})')
                plt.savefig(f'TOODIB_IBB_cls_feats_heatmap_mean_level{idx}.png')
                plt.close()
                ...
                plt.imshow(heatmap, cmap='jet')
                plt.colorbar()
                plt.title(f'TOODIB_IBB_reg_feats Heatmap mean (level {idx})')
                plt.savefig(f'TOODIB_IBB_reg_feats_heatmap_mean_level{idx}.png')
                plt.close()
                # # ===========================
                # # ...existing code...
