...
class TaskDecomposition(nn.Module):
    ...
    def forward(self,
        ...
        feat = torch.bmm(conv_weight, feat).reshape(b, self.feat_channels, h, w)#矩阵乘法
...
def print_task_decomposition_flops(feat_channels, stacked_convs, la_down_rate, conv_cfg, norm_cfg, h, w):
    ...
    flops, params = get_model_complexity_info(model, input_res, as_strings=True)
    print(f'TOOD_TaskDecomposition FLOPs: {flops}, Params: {params}')
...
class TOODHead(ATSSHead):
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[List[Tensor]]:
        ...
        cls_scores = []
        bbox_preds = []
        for idx, (x, scale, stride) in enumerate(#得到索引，特征，缩放比例，步长
                zip(feats, self.scales, self.prior_generator.strides)):
            b, c, h, w = x.shape#得到特征的形状，batch，通道，高，宽
            anchor = self.prior_generator.single_level_grid_priors(#得到单个尺度的网格先验框anchor
                (h, w), idx, device=x.device)
            anchor = torch.cat([anchor for _ in range(b)])#将anchor复制b次
            ...
            # plt.colorbar()
            # plt.title(f'TOOD_Inter_feats Heatmap mean (level {idx})')
            # plt.savefig(f'TOOD_inter_feats_heatmap_mean_level{idx}.png')
            # plt.close()
            # # # ===========================
            # # # ...existing code...

            # task decomposition
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))#平均池化
            ...
            # plt.imshow(heatmap, cmap='jet')
            # plt.colorbar()
            # plt.title(f'TOOD_TD_cls_feats Heatmap mean (level {idx})')
            # plt.savefig(f'TOOD_TD_cls_feats_heatmap_mean_level{idx}.png')
            # plt.close()
            #
            ...
            # plt.imshow(heatmap, cmap='jet')
            # plt.colorbar()
            # plt.title(f'TOOD_TD_reg_feats Heatmap mean (level {idx})')
            # plt.savefig(f'TOOD_TD_reg_feats_heatmap_mean_level{idx}.png')
            # plt.close()
            # # # ===========================
            # # # ...existing code...
