...
# tood_coordatt_20_ti_50_head描述：
# 基于tood_coordattention_20_ti_10_layatt_head.py修改,将T-head的卷积换成DCNv2卷积
# 这次希望做一个it不反传的版本，即在训练时不反传it的梯度，也不反传space_weight的梯度，只反传head的梯度
...
class CoordAtt(nn.Module):
    def __init__(self,
        ...
        # self.conv1 = nn.Conv2d(self.feat_channels, self.in_channels // la_down_rate, 1, padding=0)
        # self.bn1 = nn.BatchNorm2d(self.in_channels // la_down_rate)
        self.conv1 = nn.Conv2d(in_channels, in_channels // la_down_rate, 1, padding=0)
        self.bn1 = nn.BatchNorm2d(in_channels // la_down_rate)
        self.act = h_swish()

        # self.conv_h = nn.Conv2d(self.in_channels // la_down_rate, self.stacked_convs, stride=1, padding=1)
        # self.conv_w = nn.Conv2d(self.in_channels // la_down_rate, self.stacked_convs, stride=1, padding=1)
...
class TaskDecomposition(nn.Module):
    ...
    def forward(self,
        ...
        #                                             w)
        feat = self.reduction_conv(feat)  # 上面的conv_weight和feat的bmm操作等价于这一步
...
class TOOD_COORDATT_20_TI_50_Head(ATSSHead):
    ...
    def __init__(self,
                 num_classes: int,
                 in_channels: int,
                 num_dcn: int,      # DCN的数量，int=0则不使用DCN。
    ...
    def _init_layers(self) -> None:
        ...
        # self.kernel_width = 3                                   
        # self.dcn_weight_cls = torch.nn.Parameter(torch.Tensor(self.feat_channels, self.feat_channels // self.groups, self.kernel_height, self.kernel_width))
        # self.dcn_weight_reg = torch.nn.Parameter(torch.Tensor(self.feat_channels, self.feat_channels // self.groups, self.kernel_height, self.kernel_width))
        
        # self.dcn_offset_conv_cls = nn.Conv2d(self.feat_channels, 2 * 3 * 3, 3, 1, 1)
        # self.dcn_offset_conv_reg = nn.Conv2d(self.feat_channels, 2 * 3 * 3, 3, 1, 1)

        # self.cls_dcn_BN = nn.BatchNorm2d(self.feat_channels)
        # self.reg_dcn_BN = nn.BatchNorm2d(self.feat_channels)
        # self.relu_dcn_cls = nn.ReLU()
        # self.relu_dcn_reg = nn.ReLU()
    ...
    def init_weights(self) -> None:
        ...
        # # init dcn
        # torch.nn.init.kaiming_uniform_(self.dcn_weight_cls, a=math.sqrt(5))  # 使用 Kaiming 初始化
        # torch.nn.init.kaiming_uniform_(self.dcn_weight_reg, a=math.sqrt(5))  # 使用 Kaiming 初始化
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[List[Tensor]]:
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            # 计算TaskIter的输出，这个过程会计算梯度
            cls_space_weight = self.relu(self.TaskIter(cls_feat.detach()))
            reg_space_weight = self.relu(self.TaskIter(reg_feat.detach()))

            cls_feat = cls_feat.detach() * cls_space_weight + cls_feat
            reg_feat = reg_feat.detach() * reg_space_weight + reg_feat
            # print('self.TaskIter:', self.TaskIter.weight)

            # # DCN
            # cls_offset = self.dcn_offset_conv_cls(cls_feat)
            # reg_offset = self.dcn_offset_conv_reg(reg_feat)
       
            # cls_feat = deform_conv2d(cls_feat, cls_offset, self.dcn_weight_cls, 1, 1, 1, 1, 1)
            # reg_feat = deform_conv2d(reg_feat, reg_offset, self.dcn_weight_reg, 1, 1, 1, 1, 1)
            # cls_feat = self.relu_dcn_cls(self.cls_dcn_BN(cls_feat))
            # reg_feat = self.relu_dcn_reg(self.reg_dcn_BN(reg_feat))
